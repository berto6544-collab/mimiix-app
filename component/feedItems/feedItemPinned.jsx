...
export default function FeedItemPinned({data,navigation,loaded,setPostId,setAdUnitId,rewarded,setStatus,dataSource,setDataSource,index,Auth,isProfile}){
 ...
 const Delete = () =>{
   ...
   <View style={[FeedItemstyles.AvatarBase,{position:'relative'}]}>
    <View style={{width:'100%',paddingHorizontal:5,display:'flex',flexDirection:'column',alignItems:'flex-start',gap:5,paddingBottom:10}}>
    <Text  style={{fontSize:14,fontWeight:'500'}}>Pinned By <Text onPress={()=>{
       if(data?.PinnedUserName == data?.MyUserName){

        navigation.navigate('Profile',{username:data.PinnedUserName, isMine:true})

    }else{
        navigation.navigate('Profile',{username:data.PinnedUserName, isMine:false})

    }
    }} style={{fontSize:14,color:'blue',fontWeight:'500'}}>@{data.PinnedUserName}</Text></Text>
    
    <Text style={{fontSize:14,fontWeight:'500'}}>Pinned Until {data.FinishDate}</Text> 
    </View>
